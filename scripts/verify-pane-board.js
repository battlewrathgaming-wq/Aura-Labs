...
function main() {
  ...
  assert(main.includes("renderer', 'pane-board', 'index.html'"), 'Pane Board should load a separate renderer entry');
  assert(main.includes('aura:pane-board:load'), 'main process should expose Pane Board load IPC');
  assert(main.includes('aura:pane-board:save'), 'main process should expose Pane Board save IPC');
  assert(main.includes('aura:pane-board:snapshot'), 'main process should expose Pane Board snapshot IPC');
  assert(main.includes('aura:pane-board:export-png'), 'main process should expose Pane Board PNG export IPC');
  assert(main.includes('board-events.ndjson'), 'Pane Board should append an event log');
  assert(main.includes('Agent proposals must include basedOn.'), 'agent proposals should require basedOn');
  assert(main.includes('capturePage'), 'Pane Board should export PNG through Electron capturePage');
  assert(main.includes('pane-board-smoke-result.json'), 'Pane Board smoke should write a result artifact');
  ...
  assert(preload.includes('auraPaneBoard'), 'preload should expose a narrow Pane Board API');
