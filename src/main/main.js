...
if (process.env.AURA_LAB_ELECTRON_VISUAL_SMOKE === '1' || isPaneBoardSmokeMode()) {
...
function createWindow() {
  if (isPaneBoardMode()) {
    const { createPaneBoardWindow } = paneBoardTooling();
...
function paneBoardTooling() {
  return require('./labTooling/paneBoard/paneBoard');
}
...
app.whenReady().then(() => {
  ...
  if (isPaneBoardMode()) {
    paneBoardTooling().registerPaneBoardHandlers(ipcMain, () => mainWindow);
  }
  createWindow();
