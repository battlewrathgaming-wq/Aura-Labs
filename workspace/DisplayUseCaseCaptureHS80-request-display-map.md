...
| Sense | 0 | Under cap of 5 | Pointer and inventory exist, but Sense explicitly has no active Lab requests yet. |
...
| `sense.clipboard-window` | Sense / Sense Overseer | parked candidate from inventory | Clipboard Acquisition authority window; not submitted | Candidate needs a separate scoped `request_display` before Lab active review. | Clipboard Acquisition widget with Pulling, Listening, Cooldown, Idle, and key chips. | Preserve short visible authority window while improving boundedness and shortcut feedback. | Clipboard Acquisition; Pulling; Listening; Cooldown | background clipboard monitoring; persistent listener; hidden scan | lifecycle state; shortcut status; live IO block; capture result; cooldown/timeout/rejection reason | Needs lifecycle, authority note, live-IO blocked distinction, and timeout/cooldown. | `source-owned label`; `state label`; `authority note`; `availability note`; `effect note`; `readout age`; `narrow summary` | Status band; action readiness strip; progress strip only for real lifecycle; availability reason treatment; detail reveal | `availability-distinction`; `status-chip-row`; `collision-language`; `gap-warning-pressure` | Authority-window strip. | Lifecycle stepper. | Compact command-status card. | Listening can imply background monitoring if not time-bound and visible. | Parked until Sense submits scoped request. | Sense should decide preserve-exact vs Lab-translatable lifecycle labels before request. |
...
- Sense supplied `F:\Projects\AURA-Sense\workspace\display_inventory.md` with parked candidate rows and no active requests.
...
- Sense request rows until Sense submits scoped `request_display` entries.
...
- Five-active-request cap followed: yes; Atlas 3, Sense 0.
