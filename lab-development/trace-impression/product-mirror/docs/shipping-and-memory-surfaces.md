...
| `context.template.md` | Neutral starting template for project-local trace behavior, source boundaries, terms, avoid areas, and must-never-infer rules. | yes | Consuming projects adapt this into `workspace/trace-impression/context.md`. |
| `context.md` | Project-local trace behavior after deployment. | no as a product default | Must be created and locally calibrated by the consuming project. |
