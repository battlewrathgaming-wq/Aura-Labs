...
This candidate bundle stores neutral local context as:

```txt
context.template.md
```

When a project consumes Trace Impression, it should copy or adapt that template as:

```txt
workspace/trace-impression/context.md
```

The consumed `context.md` must be project-filled. Do not ship Lab pilot context as a default.
